...
void PNGLogReader::getNext()
{
  if ((lastGot + 1) < static_cast<int>(frames_.size())) {
    ...
    cv::Mat rgb_image = cv::imread(info.rgb_path,cv::IMREAD_COLOR);//CV_LOAD_IMAGE_COLOR
    if (flipColors) {
      cv::cvtColor(rgb_image, rgb_image, cv::COLOR_BGR2RGB); //CV_BGR2RGB
    }
    ...
    cv::Mat depth_image = cv::imread(info.depth_path,cv::IMREAD_ANYDEPTH);//CV_LOAD_IMAGE_ANYDEPTH
    index = 0;
    ...
    cv::Mat depthfill_image = cv::imread(depth_filled_str,cv::IMREAD_ANYDEPTH);//CV_LOAD_IMAGE_ANYDEPTH
